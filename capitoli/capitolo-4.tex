...
%**************************************************************
\intro{Nel seguente capitolo verrà illustrato prima di tutto la struttura di un flusso conversazionale e successivamente il funzionamento del Azzurra Flow Engine e la conseguente generazione dei messaggi da parte del \g{bot} Azzurra e dell'utente umano}\\

%*************************************************************

\section{Cos'è}
Un elemento cardine dell'\g{architettura} del sistema \gls{AWMS} è Azzurra Flow Engine. Esso è un motore conversazionale in grado di ricevere in input flussi conversazionali, implementati attraverso configurazioni \g{JSON}, che risiedono nel \g{database} di Azzurra.io. Essi vengono mandati in input a Azzurra Flow Engine quando il \g{bot} Azzurra ne fa richiesta. Questi file sono codificati secondo una certa struttura fatta dai cosiddetti blocchi conversazionali e da altri campi che verranno illustrati in seguito. Tornando su Azzurra Flow Engine come scritto, esso riceve in input la configurazione \g{JSON} e grazie ai metodi che ha disposizione è in grado di interpretare i configurazioni \g{JSON} ricevute e di generare i messaggi che il \g{bot} Azzurra deve far visualizzare all'utente nella \emph{chat}.
...
Il campo shortcuts è il campo dedicato per le cosiddette "scorciatoie" cioè, tra le funzionalità che il \g{bot} offre all'utente c'è anche la possibilità di visualizzare un menu dove vengono mostrate tutte le funzionalità offerte dal \g{bot} e scegliere direttamente quelle eseguire in ogni momento.
...
L'Azzurra Flow Engine è l'elemento in grado di interpretare i dati contenuti nelle configurazioni \g{JSON}. Grazie a esso è possibile eseguire il corretto flusso della conversazione e generare i messaggi da mostrare nella \emph{chat} dell'applicazione mobile. 
...
Una volta che si è stabilità la connessione con Azzurra.io, attraverso un \g{WebSocket}, inizializzato il Flow Engine e infine, ricevuto la configurazione \g{JSON} contenente il flusso conversazionale, la generazione di un messaggio da parte del \g{bot} Azzurra prevede i seguenti passi:\\
\begin{enumerate}
	...
	Innanzitutto, verifica chi ha emesso il messaggio, se è stato l'utente umano o il \g{bot} Azzurra. Nel caso in cui il messaggio sia del \g{bot} Azzurra viene indicato il testo da mostrare estraendolo dal messaggio ricevuto in input e aggiunto lo \emph{sprite} di Azzurra per indicare graficamente che il messaggio viene dal \g{bot};
